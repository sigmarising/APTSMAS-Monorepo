<template>
  <n-card style="margin-bottom: 7px" :content-style="contentStyle">
    <n-p style="margin-bottom: 7px; font-weight: bold">
      {{ t("vOLAM.chartTitleFreqLocation") }}
    </n-p>
...
import { useFreqLocation } from "@/composables/VTrajectoryOLAM/Charts/useFreqLocationOption";

export default defineComponent({
  name: "OLAMChartsFreqLocation",
  components: { NCard, NP },
  props: {
    isLoading: {
      type: Boolean,
  ...
  setup(props) {
    ...
    const { t } = useI18n();
    const { handleOption, initOption } = useFreqLocation(store);
