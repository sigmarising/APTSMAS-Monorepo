...
import type { IPoetsTopK } from "@/composables/VTrajectoryOLAM/IApiData";
import { NCard, NP } from "naive-ui";
import { useI18n } from "vue-i18n";
import { useStore } from "@/store";
import { usePoetTopKOption } from "@/composables/VTrajectoryOLAM/Charts/usePoetTopKOption";
...
export default defineComponent({
  ...
  props: {
    ...
    chartData: {
      type: Object as PropType<IPoetsTopK>,
      required: false,
      default: () => null,
      validator: (thing: IPoetsTopK) => thing !== undefined,
    },
  },
  ...
  setup(props) {
    ...
    const { t } = useI18n();
    const { handleOption, initOption } = usePoetTopKOption(store);
