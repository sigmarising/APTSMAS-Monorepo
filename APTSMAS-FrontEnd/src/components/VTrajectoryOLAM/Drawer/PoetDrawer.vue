...
import { usePoetData } from "@/composables/VTrajectoryOLAM/Drawer/usePoetData";
...
export default defineComponent({
  ...
  props: {
    ...
    currentSelected: {
      ...
      required: false,
      default: () => [],
      validator: (thing: string[] | number[]) => thing !== undefined,
    },
    currentText: {
      type: String,
      required: true,
  ...
  emits: {
    ...
    "update:currentSelected"(payload: string[] | number[]) {
      return payload !== undefined;
    },
  ...
  setup(props, context) {
    ...
    const drillDownVM = ref<string | number | null>(null);
    const slicingVM = ref<string[] | number[] | null>(null);
    ...
    const {
      ...
      handleRollUp,
      handleDrillDown,
    } = usePoetData(store, t, drillDownVM, slicingVM, displayText);
